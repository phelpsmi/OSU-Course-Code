...
int main(int argc, char *argv[]){
	...
	//makes sure a port number is provided
	if (argc < 2) {
...
void server_loop(int sockfd){
	...
	while(1){
		...
		if(pid == 0){
			...
			//write the encryption back to the client
...
//changes all spaces in string to brackets.
//The reason is because the bracket comes after Z
//in the asco sequence so it makes it easier to 
//deal with when encrypting
void space_to_bracket(char* s, int len){
	int i;
	for(i = 0; i < len; i++){
		if( s[i] == ' ')
...
//changes all brackets in string to spaces
//inverse function to space_to_bracket
void bracket_to_space(char* s, int len){
	int i;
	for(i = 0; i < len; i++){
		if( s[i] == '[')
