...
{
	int i;
	struct hashMap *new_map = createMap(newTableSize);
	struct hashLink * cur;
	for(i = 0; i < ht->tableSize; i++){
		cur = ht->table[i];
		while(cur){
			insertMap(new_map, cur->key, cur->value);
			cur = cur->next;
		}
	}
	*ht = *new_map;
...
void insertMap (struct hashMap * ht, KeyType k, ValueType v)
{
	...
	int i = stringHash2((char *)(k)) % ht->tableSize;
...
{
	float a = ht->count, b = ht->tableSize;
	return a/b;
}
