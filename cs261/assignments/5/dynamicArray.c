...
void addHeap(DynArr *heap, TYPE val, comparator  compare){
	...
	int next = (cur-1)/2;
	while(cur > 0){
		if((*compare)(heap->data[cur], heap->data[next]) < 0){
			swapDynArr(heap, cur, next);
			cur = next;
			next = (cur-1)/2;
		} else {
			cur = 0;
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare){
	...
	if(left > max)
		...
		if((*compare)(heap->data[pos], heap->data[left]) > 0){
			swapDynArr(heap, pos, left);
		}
	}
	else{
		if((*compare)(heap->data[left], heap->data[right]) < 0)
			smallChild = left;
		else
			smallChild = right;
		if((*compare)(heap->data[pos], heap->data[smallChild]) > 0){
